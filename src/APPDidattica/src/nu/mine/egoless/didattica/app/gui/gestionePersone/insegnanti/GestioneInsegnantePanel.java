/*
 * GestioneInsegnantePanel.java
 *
 * Created on 8 marzo 2007, 23.05
 */
 ...
 * relativi ad un insegnante
 */
public class GestioneInsegnantePanel extends GestionePersonaPanel {
    
    /**
     * Creates new form GestioneInsegnantePanel
     * @param jpnlGestioneTabStudenti il GestioneTabInsegnantiPanel in cui tale GestioneStudentiPanel è contenuto
     * @param contenitore il contenitore dei dati caricati inizialmente dall'applicazione
     */
    public GestioneInsegnantePanel(GestioneTabInsegnantiPanel jpnlAncestor, ContenitoreDati contenitore) {
        super(jpnlAncestor);
     ...
     */    
    public GestioneInsegnantePanel(GestioneTabInsegnantiPanel jpnlGestioneInsegnanti, ContenitoreDati contenitore, int idInsegnante){
        super(jpnlGestioneInsegnanti);
        if(contenitore == null) throw new IllegalArgumentException();
        this.contenitore = contenitore;
        inizializza(idInsegnante);
    }
     ...
     */        
    public int getId(){ return insegnanteBean.getId(); }    
    ...
    public void setEnabled(boolean b){
        ...
        jpnlGestioneDatiInsegnante.setEnabled(b);
    ...
    public static void main(String[] args) {
        ...
        temp.add(new GestioneInsegnantePanel(new GestioneTabInsegnantiPanel(c), c));
     ...
     */
    private void inizializza(int idInsegnante){
        insegnanteBean = new InsegnanteBean(idInsegnante);
        ...
        new SwingWorker<Object, Object>(){
            public Object doInBackground(){
                try{
                    ...
                    //CODICE RILEVANTE
                    if(insegnanteBean.getId()!=Costanti.ID_NUOVA_PERSONA)
                        caricaDatiInsegnante();
                    ...
                    getAncestor().setTesto("Caricamento terminato...");
                }catch(InterruptedException e){
                    System.out.println("GestioneInsegnantiPanel: Ecc0 "); //DEBUG CODE
                    ...
                    System.out.println("GestioneInsegnantiPanel: Ecc1 "); //DEBUG CODE
                    JOptionPane.showMessageDialog(main,"Problemi di visualizzazione");
                    getAncestor().chiudiPannelloDx(main);
                    getAncestor().setTesto("Caricamento terminato con insuccesso...");                     
                 }
                 catch(Exception e){
                    System.out.println("GestioneInsegnantiPanel: Ecc2 "); //DEBUG CODE
    ...
    /**
     * Si occupa di caricare i dati dell'eventuale insegnante (già presente nel database) da gestire (compreso il contatto)
     * Metodo d'usilio epr modularizzare
     */
    private void caricaDatiInsegnante() throws nu.mine.egoless.didattica.ws.personaclient.WSDidatticaException_Exception, nu.mine.egoless.didattica.ws.contattoclient.WSDidatticaException_Exception, PortUnreachableException {
        synchronized(insegnanteBean){
            if( insegnanteBean.getId() != Costanti.ID_NUOVA_PERSONA){            
                //System.out.println("Studente già caricato");
                System.out.println ("GestioneInsegnantePanel ID1Caricamento: "+insegnanteBean.getId ());
                //carica lo studente...
                insegnanteBean.caricaDaWS(insegnanteBean.getId());
                System.out.println ("GestioneInsegnantePanel ID2Caricamento: "+insegnanteBean.getId ());               
            }
            else{
        ...
        synchronized(contattoBean){
            ...
            int id = insegnanteBean.getIdIndirizzoResidenza();
        ...
        System.out.println("GestioneStudentePanel: Carica: # materie assegante: "+insegnanteBean.getIdMaterie().length);
    }
    
    /**
     * salva sul database i dati contenuti nella variabile insegnanteBean
     */
    private void salvaDatiInsegnante() throws nu.mine.egoless.didattica.ws.personaclient.WSDidatticaException_Exception, nu.mine.egoless.didattica.ws.contattoclient.WSDidatticaException_Exception, PortUnreachableException {
        System.out.println("GestioneStudentePanel: ID prima del salvataggio: "+insegnanteBean.getId());
        synchronized(contattoBean){
            //contattoBean.setIdPersona(insegnanteBean.getId ()); //per mantenere il riferimento alla persona giusta
            contattoBean.salvaSuWS(); //salvataggio relativo contatto 
        }        
        synchronized(insegnanteBean){           
            insegnanteBean.setIdIndirizzoResidenza (contattoBean.getId());
            this.insegnanteBean.salvaSuWS(); //salvataggio utente
            System.out.println("GestioneInsegnantePanel: ID dopo il salvataggio: "+insegnanteBean.getId());                
        }
        //System.out.println("GestioneStudentePanel: SALVATO");
        //System.out.println("GestioneStudentePanel: SALVATAGGIO: # materie assegante: "+insegnanteBean.getIdMaterie().length);
        //System.out.println("GestioneStudentePanel: SALVATAGGIO: LISTA insegnate:"); // DEBUG CODE
        //int[] ids = insegnanteBean.getIdMaterie(); //DEBUG CODE
     ...
     */
    private void cancellaDatiInsegnante() throws nu.mine.egoless.didattica.ws.personaclient.WSDidatticaException_Exception, PortUnreachableException {
        synchronized(insegnanteBean){
            insegnanteBean.cancellaInsegnante();
        }
    }
    ...
    private void salvaDebugCode(){
        String materie="";
        //Inizializzazione stringa delle materie insegnate
        //int[] ids = insegnante.getIdMaterie();
        ...
        System.out.println(
            "------------------------------\n"+
            " "+"Dati dello Studente:\n"+
            " "+insegnanteBean.getNome()+"\n"+
            " "+insegnanteBean.getCognome()+"\n"+
            " "+insegnanteBean.getDataNascita()+"\n"+
            " "+insegnanteBean.getCodiceFiscale()+"\n"+
            " "+insegnanteBean.isPortatoreHandicap()+"\n"+
            //" "+UtilitaGUI.getReligione( contenitore.getReligioni(), insegnanteBean.getIdReligione() )+"\n"+
            //" "+UtilitaGUI.getNazione( contenitore.getNazioni(), insegnanteBean.getIdNazionalita() )+"\n"+
            " "+contattoBean.getVia()+"\n"+
            ...
            " "+contattoBean.getFax()+"\n"+  
            " "+insegnanteBean.getDataAssunzione()+"\n"+
            " "+"INSERIRE LISTA DELLE MATERIE\n"+
            "------------------------------"                     
        );
    }    
    ...
    private void inizializzaComponenti(){
        try{ //DEBUG CODE
        jpnlGestioneDatiAnagrafici = new GestioneDatiAnagraficiPanel(insegnanteBean, contattoBean, contenitore);
        jpnlGestioneDatiInsegnante = new GestioneDatiInsegnantePanel(insegnanteBean, contenitore); 
        main.initComponents();                    
        jpnlAux.add(jpnlGestioneDatiAnagrafici);
        jpnlAux.add(jpnlGestioneDatiInsegnante);
        }catch(Exception e){e.printStackTrace();} //DEBUG CODE
    }        
    ...
    private void initComponents() {
        ...
        jpnlBottoniLayout.setHorizontalGroup(
            ...
            .addGroup(jpnlBottoniLayout.createSequentialGroup()
                ...
                .addGroup(jpnlBottoniLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(jprg, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, 292, Short.MAX_VALUE)
        ...
        layout.setHorizontalGroup(
            ...
            .addComponent(jpnlAux, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, 312, Short.MAX_VALUE)
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
                .addComponent(jpnlAux, javax.swing.GroupLayout.DEFAULT_SIZE, 296, Short.MAX_VALUE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
    ...
    private void jbAnteprimaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbAnteprimaActionPerformed
        AnteprimaDialog a= (new AnteprimaDialog(null, false, insegnanteBean, contattoBean, contenitore));
    ...
    private void jbSalvaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbSalvaActionPerformed
        ...
        if(insegnanteBean.getId() != Costanti.ID_NUOVA_PERSONA){
            int risposta = JOptionPane.showConfirmDialog(this, "Sei sicuro di voler salvare i dati di tale studente?\nI dati precedenti verranno sovrascritti definitivamente",
                            new String("Salvataggios Insegnante"), JOptionPane.YES_NO_OPTION);
        ...
        new SwingWorker<Object, Object>(){
            public Object doInBackground(){
                try{
                    ...
                    JOptionPane.showMessageDialog( main, "Salvataggio di "+UtilitaGUI.getStringaIdentificativa(insegnanteBean)+" avvenuto con successo", "Esito Salvataggio", JOptionPane.OK_OPTION | JOptionPane.INFORMATION_MESSAGE ); 
                }catch(Exception e){
                    JOptionPane.showMessageDialog(main, "Server non raggiungibile!\nSalvataggio di "+UtilitaGUI.getStringaIdentificativa(insegnanteBean)+" non riuscito.");
    ...
    private void jbEliminaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbEliminaActionPerformed
        if( insegnanteBean.getId() == Costanti.ID_NUOVA_PERSONA ){
            int risposta = JOptionPane.showConfirmDialog(this, "L'insegnante è nuovo! Non vi sono dati da eliminare!",
                            new String("GestioneStudentePanel: Eliminazione Insegnante Non Pertinente"), JOptionPane.YES_NO_OPTION);   
            return;                
        }
        int risposta = JOptionPane.showConfirmDialog(this, "Sei sicuro di voler eliminare i dati di tale studente?",
                        new String("Eliminazione Studente"), JOptionPane.YES_NO_OPTION);
        if(risposta == JOptionPane.NO_OPTION) return;
        (new SwingWorker<Object, Object>(){
            public Object doInBackground(){
                try{
                    ...
                    getAncestor().setUploadIndeterminato(true);      
                    //CODICE RILEVANTE
                    cancellaDatiInsegnante();
                    //CODICE RILEVANTE
                    //in teoria non dovrebbe servire eliminare il contatto!!!
                    getAncestor().setTesto("Eliminazione terminata...");                
                    getAncestor().chiudiPannelloDx(main);  
                    getAncestor().cancellaDaiRisultati(insegnanteBean.getId());                    
                    JOptionPane.showMessageDialog( main, "Eliminazione di "+UtilitaGUI.getStringaIdentificativa(insegnanteBean)+" avvenuta con successo", "Esito Eliminazione", JOptionPane.OK_OPTION | JOptionPane.INFORMATION_MESSAGE ); 
                }catch(Exception e){
                     JOptionPane.showMessageDialog(main, "Server non raggiungibile!\nEliminazione di "+UtilitaGUI.getStringaIdentificativa(insegnanteBean)+" non riuscita.");
    ...
    /** variabili locali */
    final private GestioneInsegnantePanel main = this;
    ...
    private GestioneDatiInsegnantePanel jpnlGestioneDatiInsegnante = null;
